...
// [CHANGED] Reduced to 500ms so you can check signal strength in real-time
...
void setup() {
  Serial.begin(115200);
	...
	// [FIX] Add a delay and a "Ready" message so you know the code is running
  delay(1000);
  ...
  if (esp_now_add_peer(&peerInfo) != ESP_OK){
	...
	Serial.println("ESP-NOW Initialized. Sending data...");
}

void loop() {
  // Checks if time has passed since the last transmission
